...
module.exports = {
    ...
    .then(user => {
      if(user) {
        user.passwordCompare(req.body.password)
          .then(pwMatch => {
            if (pwMatch) {
              req.locals.redirect = `/users/${user._id}`;
              req.flash(
                "success",
                 "Login success!"
              );
              ...
              req.flash(
                "error",
                 "Login failed: Password don't match."
              );
            }
             next();
           })
  ...
  valuedate: (req,res,next) => {
    req
      ...
      .trim()
      .check("email","Email is invalid") //앞/뒤 띄어쓰기 삭제
      .isEmail();

    req
      .check("passwrod", "Password cannot be empty")
      .notEmpty();
